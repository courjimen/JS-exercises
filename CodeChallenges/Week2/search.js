...
the new sentence. First argument is the sentence to perform the search and replace on.
Second argument is the word that you will be replacing (before). Third argument is what 
...
//create a str function that holds three parameters
//use replace() method 
...
function myReplace (str, word, newWord) {
    if(str.includes(word)){
        ...
        const yesCapital = newWord.charAt(0).toUpperCase() + newWord.slice(1);
        const noCapital = newWord.charAt(0).toLowerCase() + newWord.slice(1);
        //ternary operator (shorter "if else") => conditional ? true : false
        const toBeReplaced = firstCapitalized ? yesCapital : noCapital;
        const newSent = str.replace(word, newWord);
        return newSent;
    } else {
