...
expected output [{"Rogers Park": ['Cafe A', 'Cafe C', 'Cafe E']},
                 {"Wicker Park": ['Cafe B', 'Cafe D']}] 
const coffeeShops = [
...
// > 10
// <, >, ==, ===, 
const numGreaterThan10 = arrayNums.filter((number) => number > 10);
...
//  console.log(sumOfNumbers(arrayNums));
...
// ['Main Dish', 'Salad'];
                                         // acc         // { name: 'Pizza', category: 'Main Dish' },
const categoriesArr = menuItems.reduce((menuCategories, menuItem) => {
...
const coffeeShops = [
 ...
 { name: 'Cafe B', neighborhood: 'Wicker Park' },
 { name: 'Cafe C', neighborhood: 'Rogers Park' },
 { name1: 'Cafe D', neighborhood: 'Wicker Park' },
 { name: 'Cafe E', neighborhood: 'Rogers Park' },
];
//grab unique neighborhood (reduce)
                                            // acc        // currentValue
const neighborhoodArr = coffeeShops.reduce((neighborhoods, coffeeShop) => {
    if(!neighborhoods.includes(coffeeShop.neighborhood)) {
        neighborhoods.push(coffeeShop.neighborhood)
    }
    return neighborhoods
},[])
console.log(neighborhoodArr);
//display coffee shops in neighborhood w.filter